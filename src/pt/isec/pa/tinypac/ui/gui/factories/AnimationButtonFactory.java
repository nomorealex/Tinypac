...
/**
 * Class AnimationButtonFactory
 * <p>It's a factory that allows to apply a specific animation to buttons</p>
 * @author Nuno Domingues
 *
 */
public final class AnimationButtonFactory {
    private AnimationButtonFactory(){}
     ...
     * Creates an animation for the buttons provided
     * @param autoReverse if the animation needs to be reversed
     * @param buttons it's an indeterminated number of buttons that are going to have the animation
     * @return Timeline
     * */
    public static Timeline createAnimation(boolean autoReverse, Button ...buttons){
        ...
        for (var btn : buttons) {
            keyValues0.add(new KeyValue(btn.textFillProperty(), colors[0]));
            keyValues1.add(new KeyValue(btn.textFillProperty(), colors[1]));
            keyValues2.add(new KeyValue(btn.textFillProperty(), colors[2]));
            keyValues3.add(new KeyValue(btn.textFillProperty(), colors[3]));
            keyValues4.add(new KeyValue(btn.textFillProperty(), colors[4]));
        }
